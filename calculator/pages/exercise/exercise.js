...
Page({
  ...
  LungScore: function(input,gender){
    ...
    return NowScore(boys, girls, gender, input);
  },
  // 计算坐位体前屈得分
  ...
  ScoreUpdate: function() { 
    var score = 0.15*BmiScore(this.data.Height,this.data.Weight,this.data.gender) + 0.15*LungScore(this.data.LungInput,this.data.gender) + 0.2*ShortRunScore(this.data.meter50,this.data.gender) + 0.1*SitAndReach(this.data.SitandreachInput,this.data.gender) + 0.1*JumpScore(this.data.Jump,this.data.gender) + 0.1*SitupAndPullupScore(this.data.situpandpullup,this.data.gender) + 0.2*LongRunScore(this.data.meterlong,this.data.gender);
    this.setData({
      result: score
    });
    wx.setStorage({
